...
function oracle=legacyOracle(machine,raw)
...
fl=fluxlinkagefrmintAslm(fits,pitch,peakGrid,w.TurnsPerCoil,raw.CoilArea, ...
    'Skew',machine.Field.MagnetSkew,'NSkewPositions',10);
...
fl=fluxlinkagefrmintAslm(fits,pitch,lookup,w.TurnsPerCoil,raw.CoilArea, ...
    'Skew',machine.Field.MagnetSkew,'NSkewPositions',10,'Offset',oracle.peakPosition);
...
rawFit=slmengine(raw.Positions,raw.CoggingTorque/machine.ls,'EndCon','periodic', ...
    'knots',numel(raw.Positions),'Plot','off'); offsets=linspace(-machine.Field.MagnetSkew/2,machine.Field.MagnetSkew/2,10).';
cp=linspace(0,2,100); ct=(machine.ls/10)*sum(periodicslmeval(bsxfun(@plus,cp,offsets),rawFit,0,false),1);
