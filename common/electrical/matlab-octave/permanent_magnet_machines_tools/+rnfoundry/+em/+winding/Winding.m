classdef Winding
    ...
    methods
        ...
        function value = get.qc(obj), value = obj.qcn ./ obj.qcd; end
        function value = get.yp(obj), value = obj.ypn ./ obj.ypd; end
        function value = get.SlotsPerPole(obj), value = obj.SlotCount ./ obj.PoleCount; end
        function value = get.CoilsPerPhase(obj), value = obj.CoilCount ./ obj.PhaseCount; end
        function value = get.CoilsPerBranch(obj), value = obj.CoilsPerPhase ./ obj.ParallelBranches; end
        function value = get.MeanTurnLength(obj), value = obj.CoilGeometry.MeanTurnLength; end
        function value = get.TurnPathLengthPerCoil(obj), value = obj.TurnsPerCoil .* obj.MeanTurnLength; end
        function value = get.TotalTurnPathLength(obj), value = obj.CoilCount .* obj.TurnPathLengthPerCoil; end
        function value = get.TotalStrandLength(obj), value = obj.Conductor.StrandCount .* obj.TotalTurnPathLength; end
        function value = get.CopperVolume(obj), value = obj.TotalTurnPathLength .* obj.Conductor.CopperAreaPerTurn; end
        function value = get.CopperFillFactor(obj), value = obj.TurnsPerCoil .* obj.Conductor.CopperAreaPerTurn ./ obj.CoilGeometry.PackArea; end
        function value = get.ReferenceDCCoilResistance(obj), value = obj.Conductor.dcResistancePerLength() .* obj.TurnPathLengthPerCoil; end
    ...
    methods (Static)
        function obj = fromStruct(s)
            if ~isfield(s,'Type') || ~strcmp(s.Type,'Winding')
                error('rnfoundry:em:UnsupportedType','Unsupported winding type.');
            end
            if ~isfield(s.Conductor,'Type') || ~strcmp(s.Conductor.Type,'RoundWireConductor')
                error('rnfoundry:em:UnsupportedType','Unsupported conductor type in winding struct.');
            end
            if ~isfield(s.CoilGeometry,'Type') || ~strcmp(s.CoilGeometry.Type,'RadialSlottedCoilGeometry')
                error('rnfoundry:em:UnsupportedType','Unsupported coil geometry type in winding struct.');
            end
